...
#define MAX_BRACKET_IN_SENT    1000
...
char *argv[];
{
    ...
    for(Line=1;Line<skip && fgets(buff,10000,fd1)!=NULL;Line++)
	;

    for(Line=1;fgets(buff,10000,fd1)!=NULL;Line++){
	init();
	
      /* READ 1 */
	r_wn1 = read_line(buff,terminal1,&wn1,bracket1,&bn1);
      /* READ 2 */
	if(fgets(buff,10000,fd2)==NULL){
