...
#set myJarFile=/nlp/nlp-a/skulick/dbparser-myext/dbparser-ext.jar
set myJarFile = $parent/dbparser-ext.jar
echo 'myJarFile: ' $myJarFile
...
setenv classpath $myJarFile\:$jarFile\:$CLASSPATH

# the all-important output directory for this experiment
set outputDir = ~/experiments/$date/$time
...
@ numNodes=$requiredArgs[3]
setenv inFiles $requiredArgs[4]

# scripts used by this script
set startClientScript = $scriptDir/start-client.sungrid.sh
set rmiregistryScript = $scriptDir/start-rmiregistry.sungrid.sh
#set scorerScript = $scriptDir/score

# bookkeeping output files (pid files, node file script file)
set pidFile = $outputDir/pids
set rmiregistryHostFile = $outputDir/rmiregistry-host
set nodeFile = $outputDir/nodes
set scriptFile = $outputDir/$0:t

# make all switchboard output files be in output directory
setenv sbMsgFile  $outputDir/switchboard.messages
setenv outFileDir  $outputDir
setenv logFileDir  $outputDir

# arguments for switchboard java process
# set variable to indicate switchboard policy file to JVM
setenv sbPolicy  "-Djava.security.policy=$parent/policy-files/switchboard.policy"
setenv sbCodebase  "-Djava.rmi.server.codebase=file://$jarFile"
setenv disableHttp  "-Djava.rmi.server.disableHttp=true"
setenv settingsProp  "-Dparser.settingsFile=$settingsFile"
...
rm -f $rmiregistryHostFile

# start rmiregistry on switchboard host
$rmiregistryScript $rmiregistryHostFile

while ( ! -f $rmiregistryHostFile)
    echo 'Sleeping waiting for server to start'
    sleep 5
end

set host = `cat $rmiregistryHostFile`

setenv servernode `cat $rmiregistryHostFile`
...
set sbNameRMI = "//$servernode/Switchboard"

sleep 30

echo "before start client"
echo "numNodes"
#echo $numNodes
#@ numNodes = ($numNodes-1)
echo "Log dir is"
...
while ($i<4)
    ...
    qsub -cwd -j y -V -N parsingClient start-client.sungrid.sh $java $classpath $settingsFile $host $logFileDir/$i.log $outputDir -is $derivedDataFile $sbNameRMI
    @ i++
end
#echo "after start client"
